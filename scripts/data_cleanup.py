...
def clean_processed_data(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if markdown:
        ...
        cleaned_data['scrape_summary']['unique_advertisers'] = list(set(
            ad['advertiser'] for ad in unique_ads if ad.get('advertiser')
        ))
    ...
    if raw_result:
        cleaned_raw = {
            ...
            'markdown_summary': clean_markdown_text(markdown[:1000]) + "..." if len(markdown) > 1000 else clean_markdown_text(markdown)
        }
    ...
    try:
        ...
        if 'ads' in data or 'raw_result' in data:
            ...
            if 'scrape_summary' in cleaned_data:
                ...
                print(f"  📊 Found {summary['total_ads_found']} unique ads")
                print(f"  🏢 {len(summary['unique_advertisers'])} advertisers: {', '.join(summary['unique_advertisers'][:3])}{'...' if len(summary['unique_advertisers']) > 3 else ''}")
            
            return True
        print("  ⏭️ Skipping (no ad data found)")
        return False
